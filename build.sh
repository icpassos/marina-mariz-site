...
echo "==> 2/8  Dependencias PHP com dev, para testar"
cd "$APP" && "$PHP" "$COMPOSER" install --no-interaction --quiet

echo "==> 3/8  PHPUnit"
"$PHP" artisan test

echo "==> 4/8  Assets do Vite"
npm ci
npm run build

echo "==> 5/8  Dependencias PHP sem dev, otimizadas"
"$PHP" "$COMPOSER" install --no-dev --prefer-dist --optimize-autoloader --no-interaction --quiet

echo "==> 6/8  Montando $SAIDA"
rm -rf "$SAIDA"
